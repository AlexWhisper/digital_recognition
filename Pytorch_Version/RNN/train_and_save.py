...
def save_model_info(network, model_dir=None):
    ...
    if model_dir is None:
        ...
        model_dir = os.path.join(project_root, 'saved_models', 'rnn_version')
...
def main():
    print("=" * 60)
    print("MNIST Handwritten Digit Recognition Neural Network Demo (PyTorch)")
    print("=" * 60)
    ...
    learning_rate = 0.001  # Adjusted for PyTorch
    
    network = RNN(input_size=28, hidden_size=128, num_layers=2, num_classes=10, learning_rate=learning_rate)
    print(f"Network structure: input_size={{network.input_size}}, hidden_size={{network.hidden_size}}, num_layers={{network.num_layers}}, num_classes={{network.num_classes}}")
