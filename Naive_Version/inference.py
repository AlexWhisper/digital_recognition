#!/usr/bin/env python3
"""
Load and use saved neural network model for prediction (NumPy version)
"""
import numpy as np
...
def load_model_info(model_dir=None):
    ...
    biases = np.load(os.path.join(model_dir, 'model_biases.npz'))
    network.weights = [weights[f'arr_{i}'] for i in range(len(network.layer_sizes)-1)]
    network.biases = [biases[f'arr_{i}'] for i in range(len(network.layer_sizes)-1)]
...
def main():
    ...
    for i in range(num_examples):
        ...
        image = X_test[idx, :]
        true_class = np.argmax(y_test[idx, :]) if len(y_test.shape) > 1 else y_test[idx]
        # Make prediction
