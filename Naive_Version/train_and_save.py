...
def save_model_info(network, model_dir='./saved_models'):
    ...
    network.save(os.path.join(model_dir, 'model'))
    print(f"Model saved to {model_dir}/")
    print(f"   - Model structure: model_structure.json")
    print(f"   - Weights: model_weights.npz")
    print(f"   - Biases: model_biases.npz")

def main():
    print("=" * 60)
    print("MNIST Handwritten Digit Recognition Neural Network Demo (NumPy)")
    print("=" * 60)
    # 1. Load MNIST data
    ...
    # Data is already in numpy format: (num_samples, features)
    ...
    layer_sizes = [784, 128, 64, 10]  # Input 784 dim, two hidden layers, output 10 dim
    learning_rate = 0.001
    network = NeuralNetwork(layer_sizes, learning_rate)
    print(f"Network structure: {network.layer_sizes}")
